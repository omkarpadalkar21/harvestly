...
export const SellerOrdersView = () => {
  ...
  const tabs = ["all", "pending", "confirmed", "processing", "dispatched", "delivered", "cancelled", "refunded"];

  if (isLoading) {
    return <div className="p-8 text-center bg-white min-h-screen">Loading orders...</div>;
  }
  ...
  return (
    <div className="min-h-screen bg-neutral-50 p-6 lg:p-12">
      <div className="max-w-7xl mx-auto">
        ...
        <p className="text-neutral-500 mb-8">Manage incoming orders for all your products.</p>
                  ...
                  filteredOrders.map((order: any) => {
                    ...
                    return (
                      <tr key={order.id} className="hover:bg-neutral-50/50 transition-colors">
                        ...
                        <td className="px-5 py-4 align-top">
                          <p className="font-medium text-black line-clamp-2">{product?.name || "Unknown Product"}</p>
                          <p className="text-xs text-neutral-500 mt-1">Qty: {order.quantity || 1}</p>
                        </td>
                        <td className="px-5 py-4 align-top">
                          <p className="font-medium">{customer?.username || "Guest"}</p>
                          {customer?.email && <p className="text-xs text-neutral-500">{customer.email}</p>}
                        </td>
                        <td className="px-5 py-4 align-top font-semibold">
                          ₹{((product?.price || 0) * (order.quantity || 1)).toLocaleString("en-IN")}
                        </td>
                        <td className="px-5 py-4 align-top">
                          {addr ? (
                            <div className="text-xs text-neutral-600 space-y-0.5">
                              <p className="font-medium text-black">{addr.fullName}</p>
                              <p className="line-clamp-2">{addr.addressLine1} {addr.addressLine2}</p>
                              <p>{addr.city}, {addr.state} - {addr.pincode}</p>
                              <p>{addr.phone}</p>
                            </div>
                          ) : (
                            <span className="text-xs italic text-neutral-400">No address collected</span>
                          )}
                        </td>
                        <td className="px-5 py-4 align-top">
                          <Select
                            ...
                            onValueChange={(val) => {
                              // eslint-disable-next-line @typescript-eslint/no-explicit-any
                              updateStatus.mutate({ orderId: order.id, status: val as any });
                            }}
                          >
